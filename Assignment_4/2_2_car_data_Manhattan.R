library(dummies)
#filename_train_list = c("car.data.txt_train_1.csv")
#data = read.csv("car.data.txt_train_1.csv")
...
#filename_train_list = c("car.data.txt_train_1.csv")
#filename_test_list = c("car.data.txt_test_1.csv")
  ...
  for(all_files in 1:length(filename_train_list)){
    ...
    for(i in 1:no_rows_test){
      ...
      for(j in 1:no_rows_train){
        distance = dist(rbind(input_test[i:i,1:21],input_train[j:j,1:21]),method = "manhattan")
      ...
      for(ll in 1:length(min_indices_list)){
        ...
        else if(actual_train_labels[min_indices_list[ll]] == 2){
          two_count = two_count + 1
        }
        else if(actual_train_labels[min_indices_list[ll]] == 3){
      ...
      if(one_count == max(one_count, two_count, three_count, four_count)){
        predicted_test_labels = c(predicted_test_labels, 1)
      }
      
      else if(two_count == max(one_count, two_count, three_count, four_count)){
        predicted_test_labels = c(predicted_test_labels, 2)
      }
      else if(three_count == max(one_count, two_count, three_count, four_count)){
        predicted_test_labels = c(predicted_test_labels, 3)
      }
      else{
        predicted_test_labels = c(predicted_test_labels, 4)
      }
